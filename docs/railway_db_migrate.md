...
```bash
pg_dump <db-url> promptlab_db_dump.sql.tar -F t
```
...
- untar the file:
```bash
tar -xf promptlab_db_dump.sql.tar -C db_dump_extracted 
```

- run the postgres docker container:
```bash
# make sure nothing is listening on port 5432\
docker run --name promptlab-postgres \
 -e POSTGRES_PASSWORD=<put-your-pass-here> \
 -e PGDATA=/var/lib/postgresql/data/pgdata \
 -v /home/magedsaeed/Projects/promptlab/data/postgres:/var/lib/postgresql/data \
 -p 5432:5432 \
 -d postgres
```
...
```bash
docker exec -it promptlab-postgres psql -U postgres -c "CREATE DATABASE railway;"
```

- copy the extracted files to the container:
```bash
docker cp db_dump_extracted/ promptlab-postgres:/tmp/
```

- use `pg_restore`:
```bash
docker exec -it promptlab-postgres bash -c "cd /tmp/db_dump_extracted && pg_restore -U postgres -d railway ."
```
